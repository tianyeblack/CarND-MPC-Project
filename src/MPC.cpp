 ...
 public:
  ...
  void operator()(ADvector &fg, const ADvector &vars) {
    ...
    for (size_t t = 1; t < N; ++t) {
      ...
      fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 * delta * dt / Lf);
    }
  }
};
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
   ...
   * For example: If the state is a 4 element vector, the actuators is a 2 element vector and there are 10 timesteps. The number of variables is: 4 * 10 + 2 * 9
   */
  // number of independent state variables and N timesteps == N - 1 actuations
  ...
  Dvector vars(n_vars);
  for (size_t i = 0; i < n_vars; ++i) vars[i] = 0;
  ...
  // Lower and upper limits for the constraints, which should be 0 besides initial state.
  Dvector constraints_lowerbound(n_constraints);
  Dvector constraints_upperbound(n_constraints);
  for (size_t i = 0; i < n_constraints; ++i) constraints_lowerbound[i] = constraints_upperbound[i] = 0;
  ...
  /**
   * Return the first actuator values and predicted x, y coordinates
   */
